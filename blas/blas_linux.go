...
// Sgemm computes C = alpha*A*B + beta*C via OpenBLAS cblas_sgemm (row-major).
func Sgemm(transA, transB bool, M, N, K int,
	...
	// When K == 0, cblas_sgemm still needs to scale C by beta; OpenBLAS handles this.
...
// Saxpy computes y += alpha*x over n elements via OpenBLAS cblas_saxpy.
